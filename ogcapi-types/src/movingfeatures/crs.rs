...
#[serde(tag = "type")]
pub enum Crs {
    Name { properties: NamedCrs }, 
    Link { properties: LinkedCrs },
}
...
impl TryFrom<Crs> for common::Crs {
    ...
    fn try_from(value: Crs) -> Result<Self, Self::Error> {
        match value {
            ...
            Crs::Name { properties } => Self::from_str(properties.name.as_str()),
            Crs::Link { properties } => Self::from_str(properties.href.as_str()),
        }
    }
}
